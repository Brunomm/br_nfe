...
describe BrNfe::Service::Ginfes::V1::ConsultaSituacaoLoteRps do
	subject             { FactoryGirl.build(:service_ginfes_v1_consulta_situacao_lote_rps, emitente: emitente) }
	let(:emitente)      { FactoryGirl.build(:service_emitente) }

	it { must validate_presence_of(:protocolo) }

	describe "superclass" do
		it { subject.class.superclass.must_equal BrNfe::Service::Ginfes::V1::Base }
	end

	describe "#method_wsdl" do
		it { subject.method_wsdl.must_equal :consultar_situacao_lote_rps_v3 }
	end

	describe "Validação do XML através do XSD" do
		let(:schemas_dir) { BrNfe.root+'/test/fixtures/service/schemas/ginfes/V1'}

		describe "Validações a partir do arquivo XSD" do
			it "Deve ser válidado pelo schema" do
				Dir.chdir(schemas_dir) do
					schema = Nokogiri::XML::Schema(IO.read('servico_consultar_situacao_lote_rps_envio_v03.xsd'))
	...
	# 	before do 
	# 		savon.mock!
	# 		stub_request(:get, subject.wsdl).to_return(status: 200, body: read_fixture('service/wsdl/ginfes/v1/nfseservice.xml') )
	# 	end
	# 	after  { savon.unmock! }

	# 	it "Quando processou o RPS com sucesso deve setar a situation com :success" do
	# 		fixture = read_fixture('service/response/ginfes/v1/consulta_situacao_lote_rps/success.xml')
	...
	# 		fixture = read_fixture('service/response/ginfes/v1/consulta_situacao_lote_rps/error.xml')
	# 		savon.expects(:consultar_situacao_lote_rps).returns(fixture)
	# 		subject.request
	# 		response = subject.response
	...
	# 		fixture = read_fixture('service/response/ginfes/v1/consulta_situacao_lote_rps/unprocessed.xml')
	# 		savon.expects(:consultar_situacao_lote_rps).returns(fixture)
	# 		subject.request
	# 		response = subject.response
	...
	# 		fixture = read_fixture('service/response/ginfes/v1/consulta_situacao_lote_rps/unreceived.xml')
	# 		savon.expects(:consultar_situacao_lote_rps).returns(fixture)
	# 		subject.request
	# 		response = subject.response
	...
	# 		fixture = read_fixture('service/response/ginfes/v1/consulta_situacao_lote_rps/fault.xml')
