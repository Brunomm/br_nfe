...
describe BrNfe::Service::Ginfes::V1::ConsultaNfsPorRps do
	subject             { FactoryGirl.build(:service_ginfes_v1_consulta_nfs_por_rps, emitente: emitente) }
	...
	describe "#method_wsdl" do
		it { subject.method_wsdl.must_equal :consultar_nfse_por_rps_v3 }
	end

	it "#soap_body_root_tag" do
		subject.soap_body_root_tag.must_equal 'ConsultarNfsePorRpsV3'
	end
		...
		describe "Validações a partir do arquivo XSD" do
			...
			it "Deve ser válido com 1 RPS com todas as informações preenchidas" do
				...
				Dir.chdir(schemas_dir) do
					schema = Nokogiri::XML::Schema(IO.read('servico_consultar_nfse_rps_envio_v03.xsd'))
	...
	# 		fixture = read_fixture('service/response/ginfes/v1/consulta_nfse_por_rps/fault.xml')
	...
	# 		response.error_messages[0][:code].must_equal 'E4'
	...
	# 		fixture = read_fixture('service/response/ginfes/v1/consulta_nfse_por_rps/nfse_simple.xml')
	# 		savon.expects(:consultar_nfse_por_rps).returns(fixture)
	# 		subject.request
	# 		response = subject.response
	...
	# 		fixture = read_fixture('service/response/ginfes/v1/consulta_nfse_por_rps/nfse_complete.xml')
	# 		savon.expects(:consultar_nfse_por_rps).returns(fixture)
	# 		subject.request
	# 		response = subject.response
