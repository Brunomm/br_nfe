...
describe BrNfe::Service::Simpliss::V1::ConsultaNfsPorRps do
	...
	describe "#request and set response" do
		...
		it "Quando a requisição voltar com erro deve setar os erros corretamente" do
			fixture = File.read(BrNfe.root+'/test/fixtures/service/response/simpliss/v1/consulta_nfse_por_rps/fault.xml')
		...
		it "Quando encontrar uma nota fiscal com as informações básicas preenchidas" do
			fixture = File.read(BrNfe.root+'/test/fixtures/service/response/simpliss/v1/consulta_nfse_por_rps/nfse_simple.xml')
			savon.expects(:consultar_nfse_por_rps).returns(fixture)
			subject.request
			response = subject.response
		...
		it "Quando encontrar uma nota fiscal com as informações completas" do
			fixture = File.read(BrNfe.root+'/test/fixtures/service/response/simpliss/v1/consulta_nfse_por_rps/nfse_complete.xml')
			savon.expects(:consultar_nfse_por_rps).returns(fixture)
			subject.request
			response = subject.response
