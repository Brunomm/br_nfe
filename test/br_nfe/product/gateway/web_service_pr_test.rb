...
describe BrNfe::Product::Gateway::WebServicePR do
	subject { BrNfe::Product::Gateway::WebServicePR.new(env: :test) }
	...
	describe 'STATUS SERVIÇO' do
		describe '#wsdl_status_servico' do
			it "para ambiente de produção" do
				...
				subject.wsdl_status_servico.must_equal 'https://nfe.fazenda.pr.gov.br/nfe/NFeStatusServico3?wsdl'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.wsdl_status_servico.must_equal 'https://homologacao.nfe.fazenda.pr.gov.br/nfe/NFeStatusServico3?wsdl'
			end
		end
		...
		describe '#operation_status_servico' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.operation_status_servico.must_equal :nfe_status_servico_nf
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.operation_status_servico.must_equal :nfe_status_servico_nf
		...
		describe '#url_xmlns_status_servico' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.url_xmlns_status_servico.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeStatusServico3'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.url_xmlns_status_servico.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeStatusServico3'
	...
	describe 'AUTORIZAÇÃO' do
		describe '#wsdl_autorizacao' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.wsdl_autorizacao.must_equal 'https://nfe.fazenda.pr.gov.br/nfe/NFeAutorizacao3?wsdl'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.wsdl_autorizacao.must_equal 'https://homologacao.nfe.fazenda.pr.gov.br/nfe/NFeAutorizacao3?wsdl'
			end
		end
		...
		describe '#url_xmlns_autorizacao' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.url_xmlns_autorizacao.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeAutorizacao3'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.url_xmlns_autorizacao.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeAutorizacao3'
	...
	describe 'RETORNO AUTORIZAÇÃO' do
		describe '#wsdl_retorno_autorizacao' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.wsdl_retorno_autorizacao.must_equal 'https://nfe.fazenda.pr.gov.br/nfe/NFeRetAutorizacao3?wsdl'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.wsdl_retorno_autorizacao.must_equal 'https://homologacao.nfe.fazenda.pr.gov.br/nfe/NFeRetAutorizacao3?wsdl'
			end
		end
		...
		describe '#operation_retorno_autorizacao' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.operation_retorno_autorizacao.must_equal :nfe_ret_autorizacao
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.operation_retorno_autorizacao.must_equal :nfe_ret_autorizacao
		...
		describe '#url_xmlns_retorno_autorizacao' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.url_xmlns_retorno_autorizacao.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeRetAutorizacao3'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.url_xmlns_retorno_autorizacao.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeRetAutorizacao3'
	...
	describe 'CONSULTA PROTOCOLO' do
		describe '#wsdl_consulta_protocolo' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.wsdl_consulta_protocolo.must_equal 'https://nfe.fazenda.pr.gov.br/nfe/NFeConsulta3?wsdl'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.wsdl_consulta_protocolo.must_equal 'https://homologacao.nfe.fazenda.pr.gov.br/nfe/NFeConsulta3?wsdl'
			end
		end
		...
		describe '#operation_consulta_protocolo' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.operation_consulta_protocolo.must_equal :nfe_consulta_nf
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.operation_consulta_protocolo.must_equal :nfe_consulta_nf
		...
		describe '#url_xmlns_consulta_protocolo' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.url_xmlns_consulta_protocolo.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeConsulta3'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.url_xmlns_consulta_protocolo.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeConsulta3'
	...
	describe 'NFE INUTILIZAÇÃO' do
		describe '#wsdl_inutilizacao' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.wsdl_inutilizacao.must_equal 'https://nfe.fazenda.pr.gov.br/nfe/NFeInutilizacao3?wsdl'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.wsdl_inutilizacao.must_equal 'https://homologacao.nfe.fazenda.pr.gov.br/nfe/NFeInutilizacao3?wsdl'
			end
		end
		...
		describe '#operation_inutilizacao' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.operation_inutilizacao.must_equal :nfe_inutilizacao_nf
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.operation_inutilizacao.must_equal :nfe_inutilizacao_nf
		...
		describe '#url_xmlns_inutilizacao' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.url_xmlns_inutilizacao.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeInutilizacao3'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.url_xmlns_inutilizacao.must_equal 'http://www.portalfiscal.inf.br/nfe/wsdl/NfeInutilizacao3'
	...
	describe 'RECEPÇÃO EVENTO' do
		describe '#wsdl_recepcao_evento' do
			it "para ambiente de produção" do
				subject.env = :production
				subject.wsdl_recepcao_evento.must_equal 'https://nfe.fazenda.pr.gov.br/nfe/NFeRecepcaoEvento?wsdl'
			end
			it "para ambiente de homologação" do
				subject.env = :test
				subject.wsdl_recepcao_evento.must_equal 'https://homologacao.nfe.fazenda.pr.gov.br/nfe/NFeRecepcaoEvento?wsdl'
			end
		end
