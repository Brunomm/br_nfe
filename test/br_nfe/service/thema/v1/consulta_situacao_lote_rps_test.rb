...
describe BrNfe::Service::Thema::V1::ConsultaSituacaoLoteRps do
	...
	describe "#request and set response" do
		...
		it "Quando processou o RPS com erros deve setar a situation com :error" do
			fixture = File.read(BrNfe.root+'/test/fixtures/service/response/thema/v1/consulta_situacao_lote_rps/error.xml')
			savon.expects(:consultar_situacao_lote_rps).returns(fixture)
			subject.request
			response = subject.response
		...
		it "Quando não processou o RPS deve setar a situation com :unprocessed" do
			fixture = File.read(BrNfe.root+'/test/fixtures/service/response/thema/v1/consulta_situacao_lote_rps/unprocessed.xml')
			savon.expects(:consultar_situacao_lote_rps).returns(fixture)
			subject.request
			response = subject.response
		...
		it "Quando não encontrar o RPS deve setar a situation com :unreceived" do
			fixture = File.read(BrNfe.root+'/test/fixtures/service/response/thema/v1/consulta_situacao_lote_rps/unreceived.xml')
			savon.expects(:consultar_situacao_lote_rps).returns(fixture)
			subject.request
			response = subject.response
		...
		it "Quando a requisição voltar com erro deve setar os erros corretamente" do
			fixture = File.read(BrNfe.root+'/test/fixtures/service/response/thema/v1/consulta_situacao_lote_rps/fault.xml')
