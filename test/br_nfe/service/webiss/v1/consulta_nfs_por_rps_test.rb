...
describe BrNfe::Service::Webiss::V1::ConsultaNfsPorRps do
	subject             { FactoryGirl.build(:service_webiss_v1_consulta_nfs_por_rps, emitente: emitente) }
	...
	describe "Validação do XML através do XSD" do
		let(:schemas_dir) { BrNfe.root+'/test/br_nfe/service/webiss/v1/xsd' }
		...
		describe "Validações a partir do arquivo XSD" do
			...
			it "Deve ser válido com 1 RPS com todas as informações preenchidas" do
				...
				# tag envelope.
				subject.stubs(:message_namespaces).returns({'xmlns' => "http:/www.abrasf.org.br/nfse.xsd"})
	...
	describe "#request and set response" do
		...
		# 	fixture = read_fixture('service/response/webiss/v1/consulta_nfse_por_rps/fault.xml')
	...
	# 		fixture = read_fixture('service/response/webiss/v1/consulta_nfse_por_rps/nfse_simple.xml')
	# 		savon.expects(:consultar_nfse_por_rps).returns(fixture)
	# 		subject.request
	# 		response = subject.response
	...
	# 		fixture = read_fixture('service/response/webiss/v1/consulta_nfse_por_rps/nfse_complete.xml')
	# 		savon.expects(:consultar_nfse_por_rps).returns(fixture)
	# 		subject.request
	# 		response = subject.response
