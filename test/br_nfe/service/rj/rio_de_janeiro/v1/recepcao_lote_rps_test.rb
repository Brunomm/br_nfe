...
describe BrNfe::Service::RJ::RioDeJaneiro::V1::RecepcaoLoteRps do
	subject        { FactoryGirl.build(:service_rj_rj_v1_recepcao_lote_rps, emitente: emitente) }
	...
	describe "#request and set response" do
		...
		it "Quando gravou o RPS com sucesso deve setar seus valores corretamente na resposta" do
			fixture = read_fixture('service/response/rj/rio_de_janeiro/v1/recepcao_lote_rps/success.xml')
			...
			response.protocolo.must_equal '00000000000000000000000000000000000000000061325095'
			response.data_recebimento.must_equal Time.parse('2017-03-01T09:06:10')
		...
		it "Quando a requisição voltar com erro deve setar os erros corretamente" do
			fixture = read_fixture('service/response/rj/rio_de_janeiro/v1/recepcao_lote_rps/error.xml')
			...
			response.error_messages[0][:code].must_equal 'E47'
			response.error_messages[0][:message].must_equal 'CPF/CNPJ do tomador inválido'
			response.error_messages[0][:solution].must_equal 'Informe o CPF/CNPJ correto do tomador.'
			response.successful_request?.must_equal true
		end
	end
end
